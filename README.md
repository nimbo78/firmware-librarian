...
│ · скачивание документов  │      │ · /ask, @mention  │
│ · ночной ingest (05:00)  │      │ · RAG-ответы      │
└────┬──────────────┬──────┘      └─────┬───────┬─────┘
     │              │ чанки+вектора     │ поиск │
     ▼              ▼                   ▼       │
 Том загрузок    kb.sqlite (sqlite-vec+FTS5) ◀──┘
 файлы + журнал      ▲                          │
 дедупликации        │ эмбеддинги               │ вопрос + контекст
                     └──────── OpenAI API ◀─────┘
```
...
- **Ночной ingest**: каждый день в `INGEST_HOUR` (по `TZ` контейнера) качалка
  ...
  режет на чанки, эмбеддит через OpenAI и пишет в `kb.sqlite`. Пропущенный
  запуск догоняется автоматически. После инжеста база бэкапится (`VACUUM INTO`).
- **Ответ на вопрос**: бот эмбеддит вопрос → гибридный поиск top-8 чанков →
  chat completion с контекстом → ответ в тот же топик с ссылками-цитатами
  `t.me/c/…`. Кулдаун — 30 секунд на пользователя.
- **Стоимость**: бэкфилл ~100–200k сообщений ≈ $0.1–0.5 однократно;
  ответ на вопрос — десятые доли цента; суточный ingest — копейки.
...
| `KB_ANSWER_CHAT_IDS` | для KB | CSV чатов, где бот отвечает |
| `EMBED_MODEL` / `EMBED_DIM` | нет (`text-embedding-3-small` / 512) | модель эмбеддингов |
| `ANSWER_MODEL` | нет (`gpt-5-mini`) | модель ответов |
...
kb_ingest.py                чанкер, эмбеддинги, пайплайн инжеста
kb_backfill.py              одноразовый бэкфилл истории (--dry-run)
...
Dockerfile                  python:3.12-slim + tzdata
