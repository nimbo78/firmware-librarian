...
- **Стоимость**: бэкфилл ~100–200k сообщений ≈ $0.1–0.5 однократно (без
  медиа-обогащения); ответ на вопрос — десятые доли цента; суточный ingest —
  копейки.
...
Huawei: модель + версия `V-R-C-SPC`. Без LLM, бесплатно.
...
Ссылки ведут на исходные посты — файл скачивается прямо из Telegram.
Каталог наполняется тремя путями: живой захват качалкой, ночной инжест,
бэкфилл истории. Работает и в группе, и в личке админа.
...
- **Команды** (только в личке, только для id из whitelist):
  `/status` — размер базы, каталог, деньги, статистика вопросов; `/events` —
  последние 20 событий; `/gaps` — вопросы без ответа; `/fw <модель>`;
  `/review` — подтверждение LLM-связок каталога; `/notify on|off`.
