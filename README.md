...
<p align="center">
  <img src="./assets/readme/hero.svg" width="100%"
       alt="firmware-librarian — история чата и папка с прошивками как база знаний: бот отвечает со ссылкой на исходное сообщение и прикладывает файл прошивки">
</p>

Два сервиса в одном `docker-compose` на домашнем NAS. Первый слушает
Telegram-чаты сообщества, скачивает прошивки и документацию. Второй отвечает
на вопросы **словами самого чата** — со ссылками на исходные сообщения — и
выдаёт нужный файл кнопкой.

Никакого «спросите у поддержки»: ответ собирается из того, что коллеги уже
обсудили, и из документации, которая лежит в той же папке.

## Как это выглядит

Вопрос в чате — обычным текстом или через `/ask`:

```text
/ask как поднять WPA3 на AC6805?

→ Включи WPA3-SAE в профиле безопасности; точкам 5700 нужен минимум R025 [1].
  На AP4050DE это не заработает — старая радиочасть [2].

  Источники:
  [1] https://t.me/c/1523091203/48210
  [2] документация: WLAN V200R025C00 — Security → WPA3
```

Ответить реплаем на это сообщение — бот продолжит диалог: уточнение
«а на R024?» он поймёт в контексте предыдущего вопроса.

Нужен файл — каталог собран из имён и содержимого:
...
📦 S5735-S
  🔀 V200 R025 (VRP)
    💿 S5735-S_V200R025C00SPC500.cc      📎
    🩹 S5735-S_V200R025SPH121.pat        📎
  📖 Документация и release notes
    S5735-S ReleaseNotes R025.pdf        📎
```

Кнопка 📎 присылает файл прямо в чат — до 2 ГБ, с проверкой контрольной
суммы перед отправкой. `/fw` без аргументов открывает навигацию по разделам,
как на support.huawei.com.

## Что под капотом

<p align="center">
  <img src="./assets/readme/pipeline.svg" width="100%"
       alt="Пост-инжест конвейер: каталог, PDF, архивы, HedEx, LLM-экстракция, бэкап — порядок задан в одном месте, kb_pipeline.py">
</p>
...
<p align="center">
  <img src="./assets/readme/section-start.svg" width="100%" alt="Быстрый старт">
</p>
...
<p align="center">
  <img src="./assets/readme/section-kb.svg" width="100%" alt="База знаний">
</p>
...
<p align="center">
  <img src="./assets/readme/section-catalog.svg" width="100%" alt="Каталог прошивок">
</p>

## Как найти и получить файл
...
<p align="center">
  <img src="./assets/readme/section-ops.svg" width="100%" alt="Эксплуатация">
</p>
